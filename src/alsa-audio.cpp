...
void AlsaAudio::prepare()
{
    snd_pcm_prepare(m_snd);
}

void AlsaAudio::clear()
{
    snd_pcm_drop(m_snd);
}

void AlsaAudio::play(Chunk &c)
{
    const int written = snd_pcm_writei(m_snd, c.m_data, c.m_dataFrames);
    if (written < 0) {
        snd_pcm_recover(m_snd, written, 1);
    }
}
